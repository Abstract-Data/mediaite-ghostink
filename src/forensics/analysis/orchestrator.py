...
from forensics.analysis.drift import (
    _load_ai_baseline_embeddings,
    _write_drift_artifacts,
    compute_ai_convergence,
    compute_baseline_similarity_curve,
    compute_drift_scores,
    compute_intra_period_variance,
    ...
    *,
    ...
    for slug in slugs:
        ...
        if len(pairs) >= 2:
            ...
            baseline_curve = compute_baseline_similarity_curve(pairs, baseline_count=20)
            intra = compute_intra_period_variance(pairs, period="month")
            ai_vecs = _load_ai_baseline_embeddings(slug, project_root)
