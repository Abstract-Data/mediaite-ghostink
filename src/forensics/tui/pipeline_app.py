...
class PipelineDashboardApp(App[None]):
    ...
    def on_worker_state_changed(self, event: Worker.StateChanged) -> None:
        if event.state not in (WorkerState.SUCCESS, WorkerState.ERROR):
            return
        w: Worker[int] = event.worker
        if event.state == WorkerState.ERROR:
            err = w.error
            self._log_event(f"[red]Worker failed: {err!r}[/]" if err else "[red]Worker failed[/]")
            return
        rc = w.result
        self._log_event(f"Pipeline worker finished (exit code {rc}). Press q to quit.")
    ...
    def _on_run_phase_start(self, phase: PipelineRunPhase) -> None:
        ...
        self._log_event(f"Phase [bold]{label}[/] started")
    ...
    def _on_run_phase_end(self, phase: PipelineRunPhase) -> None:
        ...
        try:
            ...
            logger.debug("phase widget missing for %s", phase.value, exc_info=True)
        self._log_event(f"Phase [bold]{label}[/] finished")
    ...
    def _on_survey_author_finished(self, slug: str, error: str | None) -> None:
        ...
        if self._col_notes is not None and error:
            tbl.update_cell(slug, self._col_notes, error[:80])
...
def run_dashboard_interactive(
    *,
    ...
    """Run the dashboard until the user closes the app."""
