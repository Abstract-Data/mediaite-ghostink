...
def test_serial_run_produces_byte_identical_artifacts_across_invocations(
    ...
    run_full_analysis(serial_paths, settings, max_workers=1, exploratory=True)
    run_full_analysis(parallel_paths, settings, max_workers=1, exploratory=True)
...
def test_run_metadata_top_level_lists_sort_alphabetically(
    ...
    _pin_nondeterminism(monkeypatch)
    run_full_analysis(serial_paths, get_settings(), max_workers=1, exploratory=True)
...
def test_parallel_dispatch_emits_same_per_author_artifacts(
    ...
    results = run_full_analysis(parallel_paths, settings, max_workers=workers, exploratory=True)
