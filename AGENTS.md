...
- GitNexus **`detect_changes`** is not always available via the **`npx gitnexus`** CLI in a given install; use the **GitNexus MCP** `gitnexus_detect_changes` tool when the CLI lacks it, and do not treat a missing or failing CLI `detect_changes` as the only way to satisfy a pre-commit scope check.
